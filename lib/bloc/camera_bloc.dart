...
class CameraBloc extends Bloc<CameraEvent, CameraState> {
  ...
  CameraBloc() : super(CameraInitial()) {
    on<InitializeCamera>(_onInitialized);
  ...
  Future<void> _onTakePicture(
    ...
    event.onPictuteTaken(File(file.path));
  }
    ...
    if (picked != null && state is CameraReady) {
      ...
      emit(
        (state as CameraReady).copyWith(
          imageFile: file,
          snackBarMessage: "Image picked from gallery",
        ),
      );
    }
  }
  ...
  Future<void> _onRequestPermissions(
    ...
    if (!denied && state is CameraReady) {
      emit(
        (state as CameraReady).copyWith(
          snackBarMessage: 'Izin kamera dan penyimpananan diberikan',
        ),
      );
    }
  }
